...
RUN wget https://github.com/noobaa/noobaa-operator/releases/download/v5.9.0/noobaa-linux-v5.9.0 -O /opt/workshop/bin/noobaa && \
    wget https://mirror.openshift.com/pub/openshift-v4/clients/ocp/4.9.15/openshift-client-linux-4.9.15.tar.gz -P /opt/app-root/src/ && \
    ...
    tar -xzvf /opt/app-root/src/openshift-client-linux-4.9.15.tar.gz -C /opt/workshop/bin/ && \
    rm -f /opt/app-root/src/openshift-client-linux-4.9.15.tar.gz && \
