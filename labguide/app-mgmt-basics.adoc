...
If a pod in OpenShift would need reliable storage, for example to host a database, we would need to supply **persistent** volume to the pod. This type of storage outlives the container, i.e. it persists when the pod dies.
...
Likely, depending when you ran the command you may or may not also see that the new pods as still being spawned:
...
We will look at how this storage was provisioned automatically in the background using *Red Hat Container-native Storage* later. You will also learn how to request storage as part of a template.

Suffice to say, a 1GiB GlusterFS volume has been created and made available to the pod.

Log on to the new pods (**your pod names will be different**) using the remote-shell capability of the `oc` client:
...
Next, list the content of the root directory from the perspective of the container's namespace:
