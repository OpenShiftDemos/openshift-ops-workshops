...
[WARNING]
====
Due to the type of environment we are deploying in, it is possible that the `cns-deploy` calls fails with an error like this:
...
In this case, clean up the installation with this command:
