...
environment. You will use this in link:cns-management["Container-native Storage
Management"] lab to dynamically provision storage for containerized
applications. It is provided by GlusterFS running in containers. + GlusterFS in
turn is backed by local storage devices available to the OpenShift nodes.+ This
lab requires that you completed OpenShift installation in the previous module.

### Preparation for CNS
All of the following tasks are carried out as the `root` user on the master
node. You will now get ready to perform the CNS installation.

#### Check the Prerequisites
First, on the master, verify that the CNS deployment tool is installed. + We will also use
Ansible and verify it's presence. Though not needed for CNS per se, in this lab
it will help us simplify an otherwise tedious manual configuration step.
...
There is a host group in `/etc/ansible/hosts` for the three nodes that will be
run CNS. You will also see some hosts commented out for future use. 

You should be able to ping all the CNS hosts using Ansible:
...
There is a file in the `support` folder called `configure-firewall.yaml`. This
is a short Ansible playbook that will configure the firewall on all the relevant
OpenShift nodes to allow GlusterFS-related traffic, required for CNS. Run it
with the following command:

[source]
----
ansible-playbook /opt/lab/code/support/configure-firewall.yaml
----

When the playbooks completes successfully, your output should look like the
following:
...
CNS *Pods* need access to the physical block devices on the host. OpenShift's
security model prevents that behavior by default. In order to provide the
permissions required, you will need to enable a privileged operation mode for
the *Pods*. Execute the following command, which will allow the `default`
*ServiceAccount* to use the `privileged` *SecurityContextConstraint*:
...
CNS will virtualize the locally attached block storage on the OpenShift nodes.
...
[source,json]
./opt/lab/code/support/topology.json
----
{
    "clusters": [
        {
...
[source]
----
cns-deploy -n {{CNS_NAMESPACE}} -g topology.json --admin-key '{{HEKETI_ADMIN_PW}}' --user-key '{{HEKETI_USER_PW}}'
----
...
demonstrated in the link:cns-usage["Container-native Storage Management"]
module.
...
The definition of this *Secret* is in a file
`/opt/lab/code/support/cns-secret.yaml`. It contains a base64-encoded version of
the heketi admin password that you used earlier when deploying CNS. Make sure
you are still in the `{{ CNS_NAMESPACE }}` *Project*:
...
[source]
----
oc create -f /opt/lab/code/support/cns-secret.yaml
----

#### Create the Storage Class
To tell OpenShift about the existence of a dynamic storage provider, you use a
*StorageClass*. There is a definition file
`/opt/lab/code/support/cns-storageclass.yaml` that describes the *StorageClass*
...
[source]
----
oc create -f /opt/lab/code/support/cns-storageclass.yml
----
...
*StorageClass* will use this CNS one.

More information about dynamic storage probvisioning in OpenShift can be found
in the
