...
....
[ec2-user@{{MASTER_HOSTNAME_SHORT}} ~]# hostname
{{MASTER_HOSTNAME_FQDN}}
....

First, verify that the CNS deployment tool is installed. +
We will also use ansible and verify it's presence. Though not needed for CNS in this lab it will help us simplify an otherwise tedious manual configuration step.

  [ec2-user@{{MASTER_HOSTNAME_SHORT}} ~]# yum list installed cns-deploy

The command should return the following:

  [ec2-user@{{MASTER_HOSTNAME_SHORT}} ~]$ yum list installed cns-deploy ansible
...
....
[ec2-user@{{MASTER_HOSTNAME_SHORT}} ~]# ansible cns -m ping

{{NODE1_HOSTNAME_FQDN}} | SUCCESS => {
    "changed": false,
    "ping": "pong"
}
{{NODE2_HOSTNAME_FQDN}} | SUCCESS => {
    "changed": false,
    "ping": "pong"
}
{{NODE3_HOSTNAME_FQDN}} | SUCCESS => {
    "changed": false,
    "ping": "pong"
}
....
...
This short Ansible playbook will configure the firewall on all OpenShift worker nodes to allow GlusterFS-related traffic, required for CNS. Run it with the following command:
 ...
 [ec2-user@{{MASTER_HOSTNAME_SHORT}} ~]# ansible-playbook configure-firewall.yml
...
TASK [setup] *******************************************************************
ok: [{{NODE1_HOSTNAME_FQDN}}]
ok: [{{NODE2_HOSTNAME_FQDN}}]
ok: [{{NODE3_HOSTNAME_FQDN}}]

TASK [insert iptables rules required for GlusterFS] ****************************
changed: [{{NODE1_HOSTNAME_FQDN}}]
changed: [{{NODE2_HOSTNAME_FQDN}}]
changed: [{{NODE3_HOSTNAME_FQDN}}]
...
PLAY RECAP *********************************************************************
{{NODE1_HOSTNAME_FQDN}}          : ok=3    changed=2    unreachable=0    failed=0
{{NODE2_HOSTNAME_FQDN}}          : ok=3    changed=2    unreachable=0    failed=0
{{NODE3_HOSTNAME_FQDN}}          : ok=3    changed=2    unreachable=0    failed=0
....
====
...
GlusterFS pods need access to the physical block devices on the host. Hence they need elevated permissions. Enable containers to run in privileged mode.
 ...
 [ec2-user@{{MASTER_HOSTNAME_SHORT}} ~]# oadm policy add-scc-to-user privileged -z default
