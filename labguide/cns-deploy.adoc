...
For this purpose, in this lab environment, the file topology.json has been pre-created in the home directory of `ec2-user` on the master node. Verify it has following content:
[source,json]
...
----
[ec2-user@{{MASTER_HOSTNAME}} ~]# echo -n "{{HEKETI_ADMIN_PW}}" | base64
...
Create the secret in OpenShift with the following command:
 ...
 [ec2-user@{{MASTER_HOSTNAME}} ~]# oc create -f cns-secret.yml
...
Create the `StorageClass` in OpenShift with the following command:
 ...
 [ec2-user@{{MASTER_HOSTNAME}} ~]# oc create -f cns-storageclass.yml
