...
You can now use the OpenShift UI at https://{{ MASTER_EXTERNAL_FQDN }}:8443/ (while being logged in as _fancyuser1_ in the newly created project `my-database-app`) to follow the deployment process. Alternatively watch the containers deploy like this:
