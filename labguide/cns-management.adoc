...
[TIP]
====
To list all available parameters from this template run `oc process -f
rails-app-template.yml --parameters`
====
...
[TIP]
====
This PVC has been automatically fulfilled by CNS because it's StorageClass
...
[NOTE]
====
It may take up to 60 seconds for the GlusterFS pods to be up and in _Ready_ state.
====
...
pwd
/opt/lab/support
heketi-cli topology load --json=topology-extended.json
...
[source]
----
oc create -f second-cns-storageclass.yml
----
...
[source]
----
oc create -f cns-pvc-silver.yml
----
...
[IMPORTANT]
====
`grep` for your unique cluster ID, used when creating the `StorageClass` before.
====
...
[IMPORTANT]
====
The following method will be disruptive. All data stored in the
...
[TIP]
====
In the next release of OpenShift Container Platform the openshift-ansible
