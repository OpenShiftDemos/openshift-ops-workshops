...
We could now copy this by saving the output of this command in a file and then modify the file: change the name, add `clusterid` parameter and set the `is-default-class` value to `false`. Then use `oc create -f <file.yaml>` to create the new `StorageClass` object.

Here is a shortcut using the nifty `jq` utility (a command-line JSON manipulator) to do this in a single command line.
...
Create the `StorageClass` by parsing the JSON using the `oc create` command:

----
oc create -f glusterfs-storage-second.json
----

The command should go through without errors.
