...
{{ INFRA_INTERNAL_FQDN }}    Ready     infra     1h
...
the
link:https://docs.openshift.com/container-platform/3.10/install/prerequisites.html[prerequisites]
...
The playbook takes 3-4 minutes to complete. When done verify the amount of
...
Verify the creation by using the `oc get sc` command:

----
oc get sc
----

The output shows the new `StorageClass` is available.
...
openshift_metrics_cassanda_pvc_storage_class_name={{ CNS_BLOCK_STORAGECLASS }} <4>
openshift_metrics_hawkular_hostname=metrics.{{ OCP_ROUTING_SUFFIX }} <5>
...
----
