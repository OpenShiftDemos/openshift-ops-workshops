...
The block storage service (identified by the `gluster.org/glusterblock` provisioner) provided by this second CNS cluster will be explained in the following module. For now, it's only important to know that for CNS serving storage Logging and Metrics, the CNS block storage service is the *only* supported option. +
The regular file storage service of CNS (identified by the `kubernetes.io/glusterfs` provisioner) is *not supported* for Logging and Metrics.
...
<4> The name of the StorageClass to use for the PersistentVolumeClaim, makes it use block storage from CNS
...
You will also see the storage for Cassandra being automatically provisioned from the CNS block storage service if you query the PersistentVolumeClaim objects in this project using `oc get pvc`:
...
You will also see the storage for ElasticSearch being automatically provisioned from the CNS block storage service if you query the PersistentVolumeClaim objects in this project using `oc get pvc`:
...
[IMPORTANT]
====
The block-storage service of CNS (also referred to as `gluster-block`, introduced in the next chapter) is **only** supported for Logging and Metrics as of this release. This is about to change in the near future as we qualify more workloads.
====
