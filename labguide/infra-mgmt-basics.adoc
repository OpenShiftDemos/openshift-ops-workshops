...
Administrators can use the `oadm prune` command, potentially from a cron job, to keep control over old builds, deployments and unused container images across an entire cluster.

To demonstrate this functionality login to the project `sampleapp` created in the previous link:cluster-mgmt-basics["Cluster Management"] lab.

  [cloud-user@{{MASTER_HOSTNAME}} ~]$ oc login -u system:admin -n sampleapp

List all build configs, build objects and pods in this namespace.

  [cloud-user@{{MASTER_HOSTNAME}} ~]$ oc get bc,builds,pods

Although all the build pods are marked completed, their logs will be taking up disk space on the OpenShift nodes. Each build object also takes up a small amount of space in etcd.

List all pods that, as a result of successful build, got pushed to the OpenShift Registry:

  [cloud-user@{{MASTER_HOSTNAME}} ~]$ oc describe imagestream cake-php

The `cake-php` imagestream references every `cake-php` container image successfully built. Each container iamge will be taking up disk space in the OpenShift registry.

List all deployment configurations and replication controllers:

  [cloud-user@{{MASTER_HOSTNAME}} ~]$ oc get dc,rc

Each previous application deployment holds on to a replication controller, in case a rollback is required later. Each `rc` object takes up a small amount of space in etcd.

Let's start cleaning up by trying to prune unsued images from the registry. To do this we use the `oadm prune images` command:

  [cloud-user@{{MASTER_HOSTNAME}} ~]$ oadm prune images

Since we have run this command without the `--confirm` switch it will do a dry run instead. Right now no images will be removed because all the old images are still referenced by their replication controllers.

So first you need to remove old deployments.

NOTE: On all calls to `oadm prune` the admin user can define policy around how man old items to keep (e.g. using `--keep-younger-than=` or `--keep-complete=`).
See `oadm prune -h` for more details.

Prune everything but the last 3 deployments and the  currently active one:

  [cloud-user@{{MASTER_HOSTNAME}} ~]$ oadm prune deployments --keep-complete=3 --confirm

Validate the old replication controllers should have been removed, except the currently running deployment and
