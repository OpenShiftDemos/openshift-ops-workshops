...
To see the lines run:
----
sudo grep '#openshift_metrics_' /etc/ansible/hosts
----

Remove the `#openshift_metrics_` comment prefix by running the below `sed` command:

----
sudo sed -i 's/#openshift_metrics_//g' /etc/ansible/hosts
----
