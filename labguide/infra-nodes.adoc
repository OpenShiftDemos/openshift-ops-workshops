...
you. The following scriptlet assumes you have an AWS region called
`us-east-1e` and will build and create an infra `MachineSet` for you with 3
...
----
curl https://raw.githubusercontent.com/openshift/openshift-cns-testdrive/tree/ocp4-prod/support/infra-nodes.sh | bash
----
...
joined and ready. If you're having trouble figuring out which node is the new
...
called Custom Resource Definitions (`CRD`). A CRD is exactly what it sounds
...
Kubernetes, you would create a CRD that defines what a `Foo` is and how it
works. You can then create Custom Resources (`CRs`) -- instances of your CRD.
...
router-default-587884bb98-qk6sf   1/1     Running   0          2d4h   10.128.2.6    ip-10-0-175-100.ec2.internal   <none>
router-default-587884bb98-sbsm2   1/1     Running   0          19m    10.129.4.10   ip-10-0-194-62.ec2.internal    <none>
```
...
spec:
  domain: apps.cluster-GUID.GUID.openshiftworkshop.com
status: {}
```
...
spec:
  ...
  storage:
    s3:
      bucket: image-registry-us-east-1-69d6726b9f6d4d98a8f2c471e83edd82-0e71
      encrypt: true
      region: us-east-1
status:
...
...
https://raw.githubusercontent.com/openshift/openshift-cns-testdrive/tree/ocp4-prod/support/cluster-monitoring-configmap.yaml
...
[source,bash,role="copypaste"]
----
oc create -f https://raw.githubusercontent.com/openshift/openshift-cns-testdrive/tree/ocp4-prod/support/cluster-monitoring-configmap.yaml
----
