...
[WARNING]
====
SSH into bastion is required in the following exercise
====
...
====
You will also want to clone the content folder if the bastion doesn't have it already
[source,bash,role="execute"]
----
...
[source,bash,role="execute"]
----
bash content/support/infra-nodes.sh
----
...
[source,bash,role="execute"]
----
oc create -f content/support/cluster-monitoring-configmap.yaml
----
