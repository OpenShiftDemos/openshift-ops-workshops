...
oc create configmap ca-config-map --from-file=/opt/app-root/src/content/support/ca.crt -n openshift-config
...
[source,bash,role="execute"]
----
oc adm groups sync --sync-config=/opt/app-root/src/content/support/groupsync.yaml --confirm
----
