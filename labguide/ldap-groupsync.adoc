...
wget http://idm.openshiftworkshop.com/pub/ca.crt -O $HOME/content/support/ca.crt
oc create configmap ca-config-map --from-file=$HOME/content/support/ca.crt -n openshift-config
oc apply -f $HOME/content/support/oauth-cluster.yaml
----
...
[source,bash,role="execute"]
----
cat $HOME/content/support/groupsync.yaml
----
...
[source,bash,role="execute"]
----
oc adm groups sync --sync-config=$HOME/content/support/groupsync.yaml --confirm
----
