  ...
  - challenge: true
    ...
    provider:
      ...
      attributes: <3>
        ...
        preferredUsername:
        - uid
      bindDN: <4> uid=admin,cn=users,cn=accounts,dc=unset,dc=ocp-admin,dc=aws,dc=openshifttestdrive,dc=com
      bindPassword: <5> ldapadmin
...
Why are there no *Users* found? They are clearly listed in the *Group* definition.
...
_fancy-dev_ who should have special `cluster-reader` privileges. This is a role
...
Change the policy for the `fancy-dev` *Group*:
...
Now, login as a member of `fancy-dev`:
...
or administrative access. Let's give our `teamed-app` users access to edit the
...
Now, give the `fancy-dev` group edit access to the production project:
...
Then, try `teamuser1` from the `teamed-app` group:
...
[source]
----
[root@master ~]# oc new-app docker.io/siamaksade/mapit
