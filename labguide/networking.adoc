...
[source,bash,role="copypaste"]
----
bash /home/lab-user/content/support/create-net-projects.sh
----
...
[source,bash,role="copypaste"]
----
bash /home/lab-user/content/support/test-connectivity.sh
----
...
projects can connect to eachother.
...
where this `NetworkPolicy` CR is applied. This is the equivalent of `DenyAll`
...
Note that the `podSelector` is empty which means apply to all pods in this
*Project*. Also note that the `ingress` list is empty, which means that there
are no allowed `ingress` rules defined by this `NetworkPolicy` CR.
...
[source,bash,role="copypaste"]
----
oc create -n netproj-b -f /home/lab-user/content/support/network-policy-block-all.yaml
----
...
individual or groups of pods in a project by creating a multiple
`NetworkPolicy` CRs.
...
[source,bash,role="copypaste"]
----
oc create -n netproj-b -f /home/lab-user/content/support/network-policy-allow-all-from-netproj-a.yaml
----

### Test Connectivity #3 (should work again)
Since the "allow access from `netproj-a` " NetworkPolicy has been applied,
connectivity between the pod in the `netproj-a` *Project* and the pod in the
