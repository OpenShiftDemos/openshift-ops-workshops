...
[source,bash,role="copypaste"]
----
ll /home/lab-user/content/support/cluster-workerocs-*.yaml
----

The files do not have the correct `cluster-api-cluster` label for your lab because every environment is unique. This command will find the correct value.
...
[source,bash,role="copypaste"]
----
sed -i "s/cluster-28cf-t22gs/$CLUSTERID/g" /home/lab-user/content/support/cluster-workerocs-*.yaml
----
...
[source,bash,role="copypaste"]
----
grep cluster-api-cluster /home/lab-user/content/support/cluster-workerocs-*
----
...
[source,bash,role="copypaste"]
----
oc create -f /home/lab-user/content/support/cluster-workerocs-us-east-2a.yaml
oc create -f /home/lab-user/content/support/cluster-workerocs-us-east-2b.yaml
oc create -f /home/lab-user/content/support/cluster-workerocs-us-east-2c.yaml
----
...
[source,bash,role="copypaste"]
----
oc create -f /home/lab-user/content/support/common.yaml
----
...
[source,bash,role="copypaste"]
----
oc create -f /home/lab-user/content/support/operator-openshift.yaml
...
The log for the *rook-ceph-operator* pod should show that the operator is looking for a cluster. Look for `the server could not find the requested resource (get clusters.ceph.rook.io)` in the *rook-ceph-operator* log file. This means the operator is looking for a Ceph cluster that does not exist yet.
...
[source,bash,role="copypaste"]
----
cat /home/lab-user/content/support/cluster.yaml
----
...
[source,bash,role="copypaste"]
----
oc create -f /home/lab-user/content/support/cluster.yaml
----
...
[source,bash,role="copypaste"]
----
oc create -f /home/lab-user/content/support/toolbox.yaml
----
...
[source,bash,role="copypaste"]
----
cat /home/lab-user/content/support/storageclass.yaml
----
...
[source,bash,role="copypaste"]
----
oc create -f /home/lab-user/content/support/storageclass.yaml
----
...
Select the "New Article" link.  Enter the `username` and `password` below to create articles and comments. The articles and comments are saved in a PostgreSQL database which stores its table spaces on the Ceph RBD volume provisioned using the `rook-ceph-block` *storagclass* during the application deployment.
...
[source,bash,role="copypaste"]
----
cat machineset /home/lab-user/content/support/cluster-workerocs-us-east-2a.yaml | more
----
...
One of the *machines* should have just been created in us-east-2a AZ so there are two in this AZ now. The `NAME` of your *machines* will be different than shown below. 
...
This step could take more than 5 minutes. You can see now that one of the *machinesets* has 2 machines, this is because you increased the replica count in the prior step. The *machineset* for us-east-2a should have an integer, in this case `2`, filled out for the entire row before you proceed to the next step.
...
You should now have 7 worker nodes.  You will have more if you have completed the Infrastructure Nodes and Operators lab.
...
Eventually your will see a new *OSD* pod, `rook-ceph-osd-3`, that has just been created.  
