...
The files do not have the correct `cluster-api-cluster` label for your lab because every environment is unique. This command will find the correct value. Using this correct `cluster-api-cluster` label for your lab environment modify each of the 3 *machinesets* downloaded earlier using the variable `$CLUSTERID`.
...
WARNING: If errors are presented while running the `oc create` command run `git reset --hard` in the `/home/lab-user/content/support` directory
and ensure the value of `CLUSTERID` is defined.
