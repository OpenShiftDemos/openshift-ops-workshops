...
[source,bash,role="copypaste"]
----
oc create -f /opt/lab/support/project_request_template.yaml -n openshift-config
----

This will create the *Template* object in the `openshift-config` *Project*. You can now
see the *Templates* in the `openshift-config` project with the following:
...
The default *projectRequestTemplate* is configured in the Openshift API Server configuration. This is stored in a *ConfigMap* in the `openshift-apiserver` project.  You can view the API Server configuration with the following command:
...
This configuration is configured via an Openshift operator.  To add the default project request tempalate configuration, a *CustomResource* (CR) needs to be created.  The *CustomResource* will look like:
...
[source,bash,role="copypaste"]
----
oc create -f /opt/lab/support/cr_project_request.yaml -n openshift-config
----

Once this command is run, the Openshift API Server configurations will be updated.  This can be verified by viewing the configuration:
...
Quota:
	...
	Resource									Used	Hard
	--------									----	----
	glusterfs-registry-block.storageclass.storage.k8s.io/persistentvolumeclaims	0	0
	glusterfs-storage.storageclass.storage.k8s.io/requests.storage			0	25Gi
