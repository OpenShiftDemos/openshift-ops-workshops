...
*SSH private and public keys*:  Use an existing key pair or https://access.redhat.com/documentation/en-us/red_hat_advanced_cluster_management_for_kubernetes/2.3/html/credentials/credentials#aws_cred_create[*generate a new ssh key]*** **
...
Click on “*cluster*” in the YAML window pane and locate the section where the  +
*kind: MachinePool*.   +
Double check that the worker replicas is set to 0.  +
And the following add a line at the end of the *MachinePool* section right under the** **+skipMachinePools: ++true+
...
* Select *Step 5 *review that is under Remediation is set to *Enforce. Click Submit*
