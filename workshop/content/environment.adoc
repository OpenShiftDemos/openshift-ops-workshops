...
[IMPORTANT]
====
*Check your terminal prompt before continuing!*

Your prompt should show: `[~] $`

If you see `[lab-user@bastion ~]$` you are on the bastion host - type `exit` first to return to the dashboard terminal.
====

Check that you can see the support directory:
...
If you see `support` in the output, skip ahead to the next module.

*Only if the support directory is missing*, run these commands:

[source,bash,role="execute"]
----
git clone -b ocp4-prod https://github.com/OpenShiftDemos/openshift-ops-workshops.git
bash openshift-ops-workshops/support/clone_support_folder_v2.sh
ls ~/support/
----
