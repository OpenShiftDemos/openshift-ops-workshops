...
[start=1]
1. Click on the pipelines operator on Argo CD.

2. You should see in the box to the right `home-banner`.

3. If you `click` on `home-banner` for more information you should see that it's `KIND` is a `ConsoleNotification`

4. If we go back over to the terminal we can type in the following to see it running:
...
5. We are now going to change the text that the is being deployed on the banner:
...
We are going to change the line where the text is being deployed. Please change the following:

[source,yaml]
"/tmp/oc-edit-4028263978.yaml" 22L, 1033C
----
...
spec:
