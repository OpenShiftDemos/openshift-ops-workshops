...
To examine the banner, click on the banner on Argo CD.
image::images/home-bannerbox.png[]

In the box to the right, look for home-banner.

Click on home-banner for more information. You should see that its KIND is a ConsoleNotification.

image::images/consolenotification.png[]

To see the home-banner, switch back to the terminal and type in the following command:
...
To change the text that is being deployed on the banner, type in the following command:
[source,bash,role="execute"]
oc edit consolenotification
Change the line where the text is being deployed to the following:
...
Check back on the console and see that the banner has been updated.
image::images/oops.png[]

Go back to Argo CD and notice that the resource named home-banner is now Out of Sync.
image::images/outofsync.png[]

Select the home-banner box and select DIFF. Scroll down until you see the changes made earlier. Since self-heal is turned off, the changes won't be reverted automatically.
image::images/diff.png[]

image::images/diff2.png[]

To resync the resource, click on sync.
image::images/sync.png[]

Go back to OpenShift and observe that everything is back to normal.
