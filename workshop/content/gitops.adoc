...
As an OpenShift administrator, you can use GitOps to simplify the provisioning and configuration of your cluster and its underlying infrastructure, as well as the delivery of applications to Kubernetes. GitOps automates the deployment of Kubernetes Resources, regardless their packaging (i.e. Helm, Kustomize, kpt, etc.), to the target environment by relying on Git repositories as the source of thruth (desired state). You can easily track updates to branches, tags, or lock them to a specific version of manifests at a Git commit.
By treating infrastructure and application definitions like code, GitOps simplifies the process of propagating changes in configurations across multiple clusters.
