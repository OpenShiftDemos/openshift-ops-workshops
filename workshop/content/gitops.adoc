...
if you have already please remove the Argo CD Community Operator 
...
+
2. Please ignore warning. `Proceed` to website using the `advanced options` when you see that the connection is not private.
+
image::images/connection-private-argo.png[]
[start=3]
3. You can now `login` into Argo CD UI by selecting `Log in Via OpenShift`.
+
...
Once inside the the ArgoCD interface we are going to run through the steps to dpeloy a Pipelines Operator.

1. `Click` the `new app` in the left hand corner.

2. Once the new app screen pops up please fill out the infromation as follows:
