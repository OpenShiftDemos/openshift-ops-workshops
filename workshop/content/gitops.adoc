...
2. In the box to the right, look for `home-banner`.

3. `Click` on `home-banner` for more information. You should see that its KIND is a `ConsoleNotification`

image::images/consolenotification.png[]

4. To see the `home-banner`, switch back to the terminal and type in the following command:
...
5. To change the text that is being deployed on the banner, type in the following command:
...
8. Select the `home-banner` box and select `DIFF`. Scroll down until you see red and you should see the change that was made earlier. As `self-heal` is turned off, it won't revert the changes.
...
10. Go back to OpenShift and observe that it is back to normal.

Note: Note: Keep in mind that OpenShift GitOps can automatically revert these changes by enabling self-heal. If interested in trying that, it can be found under app details.
