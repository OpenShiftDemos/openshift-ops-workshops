...
click the command below to create a Kubernetes secret named "ldap-secret" in the "openshift-config" namespace with a single data item. The data item is named "bindPassword" and its value is set to "b1ndP^ssword"
...
Click the command below to download the certificate and save it to the file 'ca.crt'
...
Click the below command to create a new ConfigMap named ca-config-map in the openshift-config namespace. The ConfigMap is populated with the contents of the file {{ HOME_PATH }}/support/ca.crt. It then applys the file.
...
Go ahead and login as a regular user: (if you get an error, wait a moment and try again) 
...
[Warning]
====
Before continuing, make sure to go to the {{ MASTER_URL }}[OpenShift Web Console] (Do not 
use the console that is built-in to the workshop). Instead, please use the 
one that you will find on your credentials page and log out by 
using the dropdown menu at the upper right where it says `kube:admin`.
