...
====
  ...
  c. On the `Install Operator` page, *select Update Channel `stable 6.1` and  and `Version 6.1.0` *.  You should also select `'Enable Operator recommended cluster monitoring on this Namespace'`. Leave all other defaults and click `Install`. Please move onto the next step when you recieve a green completed icon.
...
2. Install the `Loki Operator`:
  ...
  c. On the `Create Operator Subscription` page, select *Update Channel `stable-6.1`*. You should also select `'Enable Operator recommended cluster monitoring on this Namespace'`. Leave all other defaults
     and click `Install`. Please move onto the next step when you recieve a green completed icon.
...
[start=2]
2. Switch to the `Operators` → `Installed Operators` page.

[start=3]
3. Make sure the `All Projects` project is selected.

[start=4]
4. Select the `Loki Operator`.

[start=5]
5. Navigate to the `LokiStack` tab and click on `lokistack-dev`. 
