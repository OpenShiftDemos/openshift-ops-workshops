...
====
  ...
  c. On the `Install Operator` page, *select Update Channel `stable 6.1` and  and `Version 6.1.0` *.  Leave all other defaults unchanged and then click `Install`.
...
2. Install the `Loki Operator`:
  ...
  c. On the `Create Operator Subscription` page, select *Update Channel `stable-6.1`*. You should also select `'Enable Operator recommended cluster monitoring on this Namespace'`. Leave all other defaults
     and click `Install`. Please manually approve if asked.
...
1. This is what each command does:
  1. Create a service account for the collector
  2. Allow the collector’s service account to write data to the LokiStack CR
  3. Allow the collector’s service account to collect logs
  4. Switch to the openshift-logging project
  5. The last 3 commands assign the `collector` service account permissions to gather application, audit, and infrastructure logs in an OpenShift cluster.
