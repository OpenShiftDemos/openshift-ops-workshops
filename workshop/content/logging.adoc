...
## (DEPRECATED) OpenShift Log Aggregation (DEPRECATED)
...
1. Please ensure that the three infra nodes are present before proceeding with the next step.
...
[source,bash,role="execute"]
----
oc get persistentvolumeclaim -n openshift-logging
----

You can also use `oc get pvc -n openshift-logging` for short.
