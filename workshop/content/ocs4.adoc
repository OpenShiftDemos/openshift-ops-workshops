...
In this section, you will first validate the OCP environment has 3 worker nodes before increasing the cluster size by additional 3 worker
...
ocs-operator.v4.2.2             OpenShift Container Storage   4.2.2                Succeeded
----
...
so be patient. You will know the new OCP worker nodes are available when you have the number `2` in all columns.
...
Once they are available, you can check to see if the new OCP worker nodes have the OCS
label applied. The total of OCP nodes with the OCS label should now be six.
...
Now that you have the new instances created with the OCS label, the next step is to add more storage to the
Ceph cluster. The OCS operator will prefer the new OCP nodes with the OCS label because they
have no OCS *Pods* scheduled yet.
