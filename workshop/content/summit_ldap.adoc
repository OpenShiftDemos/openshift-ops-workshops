...
Before we get started please clone this repo:

[source,bash,role="execute"]
----
git clone https://github.com/OpenShiftDemos/openshift-ops-workshops.git
----
...
[source,bash,role="execute"]
----
wget https://certs.godaddy.com/repository/gd-class2-root.crt -O openshift-ops-workshops/support/ca.crt
----
...
Click this command to create a new ConfigMap named ca-config-map in the openshift-config namespace. The ConfigMap is populated with the contents of the file openshift-ops-workshops/support/ca.crt. It then applies the file.

[source,bash,role="execute"]
----
oc create configmap ca-config-map --from-file=openshift-ops-workshops/support/ca.crt -n openshift-config
oc apply -f openshift-ops-workshops/support/oauth-cluster.yaml
----
...
[source,bash,role="execute"]
----
cat openshift-ops-workshops/support/groupsync.yaml
----
...
[source,bash,role="execute"]
----
oc adm groups sync --sync-config=openshift-ops-workshops/support/groupsync.yaml --confirm
----
